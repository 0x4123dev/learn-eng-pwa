...
suite('unit12: structure', () => {
    test('Unit 12 exists with id, name, icon, color, and 2000 questions', () => {
        const u = env.getGrammarUnit('unit12');
        assert.truthy(u);
        assert.equal(u.id, 'unit12');
        assert.truthy(/Tenses/i.test(u.name));
        assert.equal(u.questions.length, 2000);
...
suite('unit12: question distribution per tense', () => {
    ...
    test('total questions across all 7 tenses sums to 2000', () => {
        const u = env.getGrammarUnit('unit12');
        let total = 0;
        for (const t of TENSES) {
