...
suite('gen: sw.js cache manifest', () => {
    test('ASSETS array literal parses with exactly 85 entries', () => {
        assert.equal(ASSETS.length, 85,
...
suite('gen: sw.js js/css assets exist on disk', () => {
    test('ASSETS contains 54 /js/*.js, 1 /css/*.css, 27 /img/* entries', () => {
        assert.equal(JS_ASSETS.length, 54, 'js asset count changed');
...
suite('gen: index.html script tags', () => {
    test('index.html has exactly 53 <script src> tags, all under js/', () => {
        assert.equal(SCRIPT_SRCS.length, 53,
