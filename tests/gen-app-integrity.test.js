...
suite('gen: sw.js cache manifest', () => {
    test('ASSETS array literal parses with exactly 61 entries', () => {
        assert.equal(ASSETS.length, 61,
    ...
    test('ASSETS partitions exactly into shell(4) + 27 js + 13 img entries', () => {
...
suite('gen: sw.js js/css assets exist on disk', () => {
    test('ASSETS contains 44 /js/*.js, 1 /css/*.css, 13 /img/* entries', () => {
        assert.equal(JS_ASSETS.length, 44, 'js asset count changed');
...
suite('gen: sw.js img assets exist on disk', () => {
    ...
    test('img assets are 3 root svgs + 10 pet pngs', () => {
        ...
        assert.equal(svgs.length + pets.length, IMG_ASSETS.length);
    });
});
...
suite('gen: index.html script tags', () => {
    test('index.html has exactly 43 <script src> tags, all under js/', () => {
        assert.equal(SCRIPT_SRCS.length, 43,
