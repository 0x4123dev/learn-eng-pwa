...
suite('gen: sw.js cache manifest', () => {
    test('ASSETS array literal parses with exactly 87 entries', () => {
        assert.equal(ASSETS.length, 87,
...
suite('gen: sw.js js/css assets exist on disk', () => {
    test('ASSETS contains 56 /js/*.js, 1 /css/*.css, 27 /img/* entries', () => {
        assert.equal(JS_ASSETS.length, 56, 'js asset count changed');
...
suite('gen: index.html script tags', () => {
    test('index.html has exactly 55 <script src> tags, all under js/', () => {
        assert.equal(SCRIPT_SRCS.length, 55,
