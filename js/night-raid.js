...
var NightRaid = (() => {
  ...
  function scout(target){cleanup();view='scout';raidStage={online:true,committed:false};const r=root();if(!r)return;
    ...
    startRaid(target,true,true).catch(error=>{console.warn('Night Raid start',error);if(typeof showToast==='function')showToast('Chưa thể vào trận — con thử lại nhé');showLiveTargets();});
  }
  ...
  function buildAsset(def){
    const farm=NightRaidRules.farmRules;
    if(def.kind==='crop')return farm.art(def.id+'-day'+def.days);
    if(def.kind==='plot')return farm.art(def.asset||def.id);
    return def.kind?farm.art(def.id):'img/night-raid/'+(def.asset||def.id+'.webp');
  }
  ...
  function seedState(){const raw=appState.dailyTask&&appState.dailyTask.seeds;return raw&&typeof raw==='object'?raw:{progress:0,goal:2,next:{id:'lettuce',name:'Rau cải',days:1,yield:8},inventory:[],recent:[]};}
  function seedQuantity(id){const item=(seedState().inventory||[]).find(x=>x&&x.id===id);return Math.max(0,Math.trunc(+((item&&item.quantity)||0)));}
  function totalSeeds(){return(seedState().inventory||[]).reduce((n,x)=>n+Math.max(0,Math.trunc(+((x&&x.quantity)||0))),0);}
  function adoptFarmClock(data){if(!data)return;if(typeof data.dayCount==='number'&&Number.isFinite(data.dayCount))appState.farmDayCount=Math.max(0,Math.trunc(data.dayCount));if(data.ctx&&typeof data.ctx==='object')appState.farmCtx={today:String(data.ctx.today||''),doneYesterday:!!data.ctx.doneYesterday,doneToday:!!data.ctx.doneToday};if(data.seeds&&typeof data.seeds==='object'){if(!appState.dailyTask)appState.dailyTask={};appState.dailyTask.seeds=data.seeds;}}
  ...
  // The castle is the primary board, but extra boards stay visible beside it.
  // Explicit zone arguments handle farm actions; this fallback keeps legacy
  // callers on the castle instead of silently switching the whole screen.
  function activeZone(layout){builderZone=0;return zoneOf(layout,0);}
  const SHOP_TABS=[['defense','Phòng thủ','shield'],['farm','Nông trại','farm'],['expand','Mở rộng','expand']];
  function shopItems(tab){const F=NightRaidRules.farmRules;return tab==='defense'?NightRaidRules.DEFENSES:tab==='farm'?F.FARM_BUILDINGS:[];}
  ...
  function shopCardHtml(d,layout){const capped=!!d.buyMax&&ownedCount(layout,d.id)>=d.buyMax,poor=(+appState.coins||0)<d.price;const stats=d.kind==='crop'?`<b class="producer">${d.days} ngày · +${d.yield} xu</b>`:d.kind==='farm'?`<b class="producer">Trang trí · ${d.footprint===2?'4 ô':'1 ô'}</b>`:buildStatHtml(d);
    return `<button type="button" draggable="${capped?'false':'true'}" class="nr-build-item ${selectedBuild===d.id?'selected':''} ${poor?'unaffordable':''} ${capped?'owned-max':''}" ${capped?'disabled':''} onclick="nrSelectBuild('${d.id}')" onpointerdown="nrBeginBuildDrag(event,'${d.id}')" ondragstart="nrNativeBuildDrag(event,'${d.id}')" aria-pressed="${selectedBuild===d.id}"><img class="nr-build-art" src="${buildAsset(d)}" alt="" loading="lazy" decoding="async"><span class="nr-build-copy"><strong>${esc(d.name.vi)}</strong><small class="nr-item-stats">${capped?'<b class="producer">Mỗi loại 1 cái</b>':stats}</small><span class="nr-coin-price">${svg('coin')} ${d.price} xu</span></span></button>`;}
  function seedCardHtml(d){const quantity=seedQuantity(d.id),empty=quantity<1;return `<button type="button" draggable="${empty?'false':'true'}" class="nr-build-item nr-seed-item ${selectedBuild===d.id?'selected':''} ${empty?'owned-max':''}" ${empty?'disabled':''} onclick="nrSelectBuild('${d.id}')" onpointerdown="nrBeginBuildDrag(event,'${d.id}')" ondragstart="nrNativeBuildDrag(event,'${d.id}')" aria-label="${esc(d.name.vi)}, còn ${quantity} hạt"><img class="nr-build-art" src="${buildAsset(d)}" alt="" loading="lazy" decoding="async"><span class="nr-build-copy"><strong>${esc(d.name.vi)}</strong><small class="nr-item-stats"><b class="producer">${d.days} ngày · thu ${d.yield} xu</b></small><span class="nr-seed-count">x${quantity} hạt</span></span></button>`;}
  function expandCardHtml(layout){const F=NightRaidRules.farmRules,P=F.FARM_PLOT,have=(layout.farms||[]).length,full=have>=P.max,poor=(+appState.coins||0)<P.price;return F.FARM_PLOT_STYLES.map(style=>{const def=Object.assign({},P,style,{kind:'plot'});return `<button type="button" class="nr-build-item nr-expand-item ${poor||full?'unaffordable':''}" ${full?'disabled':''} onclick="nrBuyFarmPlot('${style.id}')"><img class="nr-build-art" src="${buildAsset(def)}" alt="" loading="lazy" decoding="async"><span class="nr-build-copy"><strong>${esc(style.name.vi)}</strong><small class="nr-item-stats"><b class="producer">Đất xanh ${P.size}×${P.size} · đã có ${have}/${P.max}</b></small><span class="nr-coin-price">${svg('coin')} ${P.price} xu</span></span></button>`;}).join('');}
  function shopTabsHtml(){return `<div class="nr-shop-tabs" role="tablist" aria-label="Loại vật phẩm">${SHOP_TABS.map(([id,label,icon])=>`<button type="button" role="tab" class="${builderShopTab===id?'active':''}" aria-label="${label}" title="${label}" aria-selected="${builderShopTab===id}" onclick="nrSelectShopTab('${id}')">${svg(icon)}</button>`).join('')}</div>`;}
  function trayHtml(layout){if(builderShopTab==='seeds')return NightRaidRules.farmRules.CROPS.map(seedCardHtml).join('');if(builderShopTab==='expand')return expandCardHtml(layout);return shopItems(builderShopTab).map(d=>shopCardHtml(d,layout)).join('');}
  function zoneChipsHtml(){return'';}
  ...
  // should receive a keyboard/tap action next.
  function selectZone(zone){builderZone=Math.max(0,Math.trunc(+zone||0));}
  ...
  function yardArmyHtml(){const total=Math.max(0,Math.trunc(+appState.nightRaidLayout?.soldiers||0)),count=Math.min(NightRaidRules.ARMY_DISPLAY_CAP,total),rowStep=100/(NightRaidRules.SOLDIER_VARIANT_COUNT-1);if(!count)return'';return `<div class="nr-yard-army" data-nr-yard-army data-mode="march" data-total="${total}" aria-label="${total>count?`Đội hình ${total} lính đang duyệt binh, bãi cỏ hiện ${count} con`:`Đội hình ${count} lính đang duyệt binh trên bãi cỏ`}"><div class="nr-army-trail" data-nr-army-trail aria-hidden="true"></div>${armySlots(count).map((s,i)=>`<i class="nr-home-soldier" data-unit="${i}" data-row="${s.row}" style="--nr-slot-x:${s.x}%;--nr-slot-y:${s.y}%;--nr-unit-position:0% ${s.row*rowStep}%"><b aria-hidden="true"></b></i>`).join('')}</div>`;}
  ...
  function buildSpaceFree(layout,gx,gy,size,layer,ignoreCell,includeCastle=true,zone){
    const z=zoneOf(layout,zone===undefined?builderZone:zone),G=z.grid,candidate={gx:+gx,gy:+gy,size:+size};
  ...
  function footprintOwner(layout,gx,gy,layer,zone){return zoneOf(layout,zone===undefined?builderZone:zone).cells.find(cell=>{const box=buildRect(cell);return box.layer===layer&&gx>=box.gx&&gx<box.gx+box.size&&gy>=box.gy&&gy<box.gy+box.size;})||null;}
  ...
  function placeArmyParade(army,state,now){army.dataset.mode=state.mode;army.style.setProperty('--nr-army-x',state.x.toFixed(2)+'%');army.style.setProperty('--nr-army-y',state.y.toFixed(2)+'%');army.style.setProperty('--nr-army-dir',state.dir);army.querySelectorAll('[data-unit]').forEach((unit,i)=>{const row=Math.max(0,Math.min(NightRaidRules.SOLDIER_VARIANT_COUNT-1,+unit.dataset.row||0)),rowY=row*(100/(NightRaidRules.SOLDIER_VARIANT_COUNT-1));if(state.mode==='march'){const frame=(Math.floor(now/120)+i*2)%6;unit.style.setProperty('--nr-unit-position',(frame*20)+'% '+rowY+'%');unit.style.setProperty('--nr-gait-phase',((frame%3)-1));}else{const frame=state.mode==='attention'?6:7;unit.style.setProperty('--nr-unit-position',(frame*(100/7))+'% '+rowY+'%');unit.style.setProperty('--nr-gait-phase',0);}});}
  ...
  function farmGridHtml(layout,zone){const z=zoneOf(layout,zone),G=z.grid,cellMap=new Map(z.cells.map(c=>[gridKey(c),c]));let html='';
    for(let gy=0;gy<G;gy++)for(let gx=0;gx<G;gx++){
      const stand=cellMap.get(gx+':'+gy+':stand'),floor=cellMap.get(gx+':'+gy+':floor'),standOwner=footprintOwner(layout,gx,gy,'stand',zone),floorOwner=footprintOwner(layout,gx,gy,'floor',zone),covered=(standOwner&&!stand)||(floorOwner&&!floor),standDef=standOwner&&NightRaidRules.itemById(standOwner.type),standLabel=standDef?', '+standDef.name.vi:'';
      html+=`<button type="button" class="nr-build-grid-cell ${stand?'has-stand':''} ${floor?'has-floor':''} ${covered?'footprint-covered':''}" data-zone="${zone}" data-gx="${gx}" data-gy="${gy}" onclick="nrGridCell(${gx},${gy},${zone})" ondragover="nrBuildDragOver(event)" ondrop="nrDropBuildItem(event,${gx},${gy},${zone})" aria-label="Nông trại ${zone}, hàng ${gy+1}, cột ${gx+1}${standLabel}">${placedHtml(floor,'floor',gx,gy,zone)}${placedHtml(stand,'stand',gx,gy,zone)}<i aria-hidden="true"></i></button>`;
    }return html;
  }
  function mountUnifiedFarms(map,layout){if(!map)return;const main=map.querySelector(':scope > .nr-free-grid');if(main){main.dataset.zone='0';main.querySelectorAll('.nr-build-grid-cell').forEach(cell=>cell.dataset.zone='0');}
    ...
    (layout.farms||[]).forEach((farm,index)=>{const zone=index+1,style=NightRaidRules.farmRules.plotStyle(farm.style);map.insertAdjacentHTML('beforeend',`<section class="nr-farm-plot style-${style.id}" data-farm-zone="${zone}" data-farm-style="${style.id}" style="--nr-farm-x:${farm.x}%;--nr-farm-y:${farm.y}%" aria-label="Nông trại ${zone}, ${esc(style.name.vi)}, khu đất xanh 6 nhân 6"><img class="nr-farm-surface" src="${NightRaidRules.farmRules.art(style.asset)}" alt="" aria-hidden="true"><button type="button" class="nr-farm-plot-handle" onpointerdown="nrBeginFarmPlotDrag(event,${zone})" aria-label="Kéo Nông trại ${zone} vào một ô vuông quanh lâu đài">🌱 <span>NÔNG TRẠI ${zone} · KÉO</span></button><div class="nr-free-grid size-6" data-zone="${zone}" role="grid" aria-label="Lưới Nông trại ${zone}, 6 nhân 6">${farmGridHtml(layout,zone)}</div></section>`);});
  }
  ...
  function centerBuilderWorld(){
    ...
    const cleanLayout=NightRaidRules.normalizeLayout(appState.nightRaidLayout);
    if(view==='builder')mountUnifiedFarms(activeMap,cleanLayout);
    decorateCastleYard(activeMap,view==='builder');
  ...
  function showBuildPurchase(plan){
    pendingBuildPurchase={id:plan.id,style:plan.style,gx:plan.gx,gy:plan.gy,zone:plan.zone};document.querySelector('.nr-purchase-backdrop')?.remove();const builder=document.querySelector('.nr-builder');if(!builder)return;
    const seed=plan.def.kind==='crop',summary=seed
      ? `<div class="nr-purchase-price seed"><span>Kho hiện có</span><strong>${seedQuantity(plan.def.id)} hạt</strong></div>`
      : `<div class="nr-purchase-price"><span>${plan.remove?'Hoàn lại':'Giá'+(plan.refund?' sau hoàn xu':'')}</span><strong>${svg('coin')}${plan.remove?plan.refund:Math.max(0,plan.cost-plan.refund)} xu</strong></div><div class="nr-purchase-balance"><span>Số dư hiện tại <b>${plan.balance}</b></span><span aria-hidden="true">→</span><span>Sau khi mua <b>${plan.balanceAfter}</b></span></div>`;
    builder.insertAdjacentHTML('beforeend',`<div class="nr-purchase-backdrop" role="presentation" onclick="if(event.target===this)nrCancelBuildPurchase()" onkeydown="if(event.key==='Escape')nrCancelBuildPurchase()"><section class="nr-purchase-dialog" role="dialog" aria-modal="true" aria-labelledby="nrPurchaseTitle" aria-describedby="nrPurchaseDetail"><button class="nr-purchase-close" type="button" onclick="nrCancelBuildPurchase()" aria-label="Hủy">${svg('close')}</button><img src="${buildAsset(plan.def)}" alt=""><div class="nr-purchase-copy"><span class="nr-label">${seed?'XÁC NHẬN GIEO':'XÁC NHẬN MUA'}</span><h2 id="nrPurchaseTitle">${esc(plan.title)}</h2><p id="nrPurchaseDetail">${esc(plan.detail)}</p></div>${summary}<div class="nr-purchase-actions"><button type="button" class="nr-purchase-cancel" onclick="nrCancelBuildPurchase()">Hủy</button><button type="button" class="nr-purchase-confirm" onclick="nrConfirmBuildPurchase()">${seed?'Gieo 1 hạt':plan.remove?'Dỡ':'Xác nhận mua'}</button></div></section></div>`);requestAnimationFrame(()=>document.querySelector('.nr-purchase-confirm')?.focus());
  }
  ...
  function confirmBuildPurchase(){const pending=pendingBuildPurchase;if(!pending)return;pendingBuildPurchase=null;document.querySelector('.nr-purchase-backdrop')?.remove();if(pending.id==='farm-plot'){buyFarmPlot(pending.style,true);return;}selectedBuild=pending.id;buildCell(pending.gx,pending.gy,true,pending.zone);}
  function buildCell(gx,gy,confirmed,zone){if(!confirmed&&builderSuppressClick){builderSuppressClick=false;return;}if(!builderEditing&&!confirmed)return;const plan=buildPurchasePlan(selectedBuild,gx,gy,zone);if(!plan)return;if(plan.error){if(typeof showToast==='function')showToast(plan.error);return;}if(!confirmed){showBuildPurchase(plan);return;}if(plan.def.kind==='crop')return plantSeed(plan);if(plan.def.producer==='soldier'&&!plan.remove)return purchaseBarracks(plan);const {def,layout,at,cell,cost,refund}=plan,z=zoneOf(layout,plan.zone),G=z.grid,lane=Math.round(plan.gy*4/(G-1)),col=1+Math.round(plan.gx*7/(G-1));
  ...
  async function purchaseBarracks(plan){const synced=await syncHome();if(!synced||!synced.ok){if(typeof showToast==='function')showToast('Cần kết nối máy chủ để mua nhà lính');renderBuilder();return;}selectedBuild=plan.def.id;const fresh=buildPurchasePlan(selectedBuild,plan.gx,plan.gy,plan.zone);if(!fresh||fresh.error){if(typeof showToast==='function')showToast((fresh&&fresh.error)||'Chưa mua được nhà lính');renderBuilder();return;}const {layout,at,cell,def}=fresh,z=zoneOf(layout,fresh.zone),G=z.grid,lane=Math.round(fresh.gy*4/(G-1)),col=1+Math.round(fresh.gx*7/(G-1)),made=newBuildCell(def,lane,col,fresh.gx,fresh.gy);if(cell)z.cells[at]=made;else z.cells.push(made);const candidate=NightRaidRules.normalizeLayout(layout),res=await api('home',{method:'PUT',body:{layout:candidate,dogLevel:appState.dogLevel,castleSkin:appState.petBattleCastleSkin||'stone-keep',barracksPurchase:{uid:made.uid,gx:fresh.gx,gy:fresh.gy}}});if(!res.ok||!res.data){if(typeof showToast==='function')showToast((res.data&&res.data.error)||'Chưa mua được nhà lính, thử lại nhé');renderBuilder();return;}adoptFarmClock(res.data);appState.nightRaidLayout=NightRaidRules.normalizeLayout(res.data.layout);if(typeof res.data.coins==='number')appState.coins=Math.max(0,Math.trunc(res.data.coins));setHomeDirty(false);builderEditing=false;builderShopOpen=false;save();if(typeof showToast==='function')showToast('Đã mua '+def.name.vi+' · chung tiến độ với nhà lính đầu tiên · còn '+Math.floor(appState.coins)+' xu');announce('Đã mua '+def.name.vi);renderBuilder();}
  async function plantSeed(plan){const res=await api('plant',{method:'POST',body:{cropId:plan.def.id,zone:plan.zone,gx:plan.gx,gy:plan.gy}});if(!res.ok||!res.data){if(typeof showToast==='function')showToast((res.data&&res.data.error)||'Chưa gieo được hạt, thử lại nhé');renderBuilder();return;}adoptFarmClock(res.data);appState.nightRaidLayout=NightRaidRules.normalizeLayout(res.data.layout);builderEditing=false;builderShopOpen=false;save();if(typeof showToast==='function')showToast('Đã gieo 1 hạt '+plan.def.name.vi+' · còn '+seedQuantity(plan.def.id)+' hạt');announce('Đã gieo '+plan.def.name.vi);renderBuilder();}
  function toggleBuilderGrid(){builderEditing=!builderEditing;if(!builderEditing)builderShopOpen=false;renderBuilder();}
  function openSeeds(){builderShopTab='seeds';builderShopOpen=true;builderEditing=false;selectedBuild=null;renderBuilder();}
  function toggleBuildShop(){const opening=!builderShopOpen;builderShopOpen=opening;if(opening&&builderShopTab==='seeds')builderShopTab='defense';builderEditing=false;renderBuilder();}
  ...
  function newBuildCell(def,lane,col,gx,gy){const uid=(p)=>p+Date.now().toString(36)+Math.random().toString(36).slice(2,10);
    ...
    const cell={type:def.id,lane,col,gx,gy,tier:1};if(def.producer){cell.uid=uid('p-');if(def.perTaskDay){const leader=NightRaidRules.farmRules.allCells(appState.nightRaidLayout).find(c=>NightRaidRules.itemById(c.type)?.producer==='soldier');cell.lastDay=leader&&Number.isFinite(+leader.lastDay)?leader.lastDay:farmDay();cell.soldierCycles=Math.max(0,Math.trunc(+(leader&&leader.soldierCycles)||0));}else cell.readyAt=Date.now()+def.productionMs;}return cell;}
  ...
  function setupBuilderGestures(){const viewport=document.getElementById('nrBuilderWorld');if(!viewport)return;
    ...
    viewport.addEventListener('click',e=>{
      ...
      const grid=[...viewport.querySelectorAll('.nr-free-grid')].find(candidate=>{const box=candidate.getBoundingClientRect();return e.clientX>=box.left&&e.clientX<=box.right&&e.clientY>=box.top&&e.clientY<=box.bottom;});if(!grid)return;
      const rect=grid.getBoundingClientRect();
      const fx=(e.clientX-rect.left)/rect.width,fy=(e.clientY-rect.top)/rect.height;
      const zone=Math.max(0,Math.trunc(+grid.dataset.zone||0)),G=zone?NightRaidRules.farmRules.FARM_PLOT.size:NightRaidRules.BUILD_GRID,gx=Math.floor(fx*G),gy=Math.floor(fy*G);
  ...
  function movePlacedItem(fromGx,fromGy,layer,toGx,toGy,zone){fromGx=+fromGx;fromGy=+fromGy;toGx=+toGx;toGy=+toGy;zone=Math.max(0,Math.trunc(+zone||0));settleBuilderPlacement();if(fromGx===toGx&&fromGy===toGy)return;const layout=NightRaidRules.normalizeLayout(appState.nightRaidLayout),z=zoneOf(layout,zone),G=z.grid,sameLayer=c=>(NightRaidRules.itemById(c.type).trap?'floor':'stand')===layer,source=z.cells.find(c=>c.gx===fromGx&&c.gy===fromGy&&sameLayer(c));if(!source)return;const size=NightRaidRules.footprintFor(source.type);toGx=Math.max(0,Math.min(G-size,toGx));toGy=Math.max(0,Math.min(G-size,toGy));if(!buildSpaceFree(layout,toGx,toGy,size,layer,source,true,zone)){if(typeof showToast==='function')showToast(size===2?'Cần một vùng trống 2 × 2 ô':'Ô này đã có công trình');return;}source.gx=toGx;source.gy=toGy;source.lane=Math.round(toGy*4/(G-1));source.col=1+Math.round(toGx*7/(G-1));rememberBuilderWorld();appState.nightRaidLayout=NightRaidRules.normalizeLayout(layout);builderSuppressClick=false;save();syncHome();renderBuilder();announce('Đã chuyển công trình sang vị trí mới');}
  function beginPlacedDrag(event,gx,gy,layer,zone){if(!builderEditing)return;event.preventDefault();const src=event.currentTarget.currentSrc||event.currentTarget.src;smoothBuilderDrag(event,src,()=>{},target=>{builderSuppressClick=true;const targetZone=+target.dataset.zone||0;if(targetZone!==Math.max(0,+zone||0)){if(typeof showToast==='function')showToast('Hãy kéo công trình trong đúng khu đất của nó');return;}movePlacedItem(gx,gy,layer,+target.dataset.gx,+target.dataset.gy,targetZone);});}
  function beginFarmPlotDrag(event,zone){if(!builderEditing||(event.pointerType==='mouse'&&event.button!==0))return;event.preventDefault();event.stopPropagation();zone=Math.max(1,Math.trunc(+zone||0));const handle=event.currentTarget,plot=handle.closest('.nr-farm-plot'),map=plot?.closest('.nr-builder-map');if(!plot||!map)return;
    const layout=NightRaidRules.normalizeLayout(appState.nightRaidLayout),farm=layout.farms[zone-1];if(!farm)return;const pointerId=event.pointerId,startX=event.clientX,startY=event.clientY,start={x:farm.x,y:farm.y},used=new Set(layout.farms.filter((_,index)=>index!==zone-1).map(f=>f.x+':'+f.y));let x=start.x,y=start.y,moved=false,valid=true;
    const highlight=(dock,isValid)=>{map.querySelectorAll('.nr-farm-dock').forEach(el=>el.classList.toggle('active',+el.dataset.farmDock===NightRaidRules.FARM_PLOT_DOCKS.indexOf(dock)));plot.classList.toggle('invalid',!isValid);};
    try{handle.setPointerCapture(pointerId);}catch(_){}plot.classList.add('dragging');map.classList.add('farm-dragging');map.querySelectorAll('.nr-farm-dock').forEach(el=>{const dock=NightRaidRules.FARM_PLOT_DOCKS[+el.dataset.farmDock];if(dock&&dock.x===start.x&&dock.y===start.y)el.classList.add('source');});
    const move=e=>{if(e.pointerId!==pointerId)return;const rect=map.getBoundingClientRect();if(Math.hypot(e.clientX-startX,e.clientY-startY)>4)moved=true;if(!moved)return;const rawX=start.x+(e.clientX-startX)/Math.max(1,rect.width)*100,rawY=start.y+(e.clientY-startY)/Math.max(1,rect.height)*100,dock=NightRaidRules.nearestFarmPlotDock(rawX,rawY);x=dock.x;y=dock.y;valid=!used.has(x+':'+y);plot.style.setProperty('--nr-farm-x',x+'%');plot.style.setProperty('--nr-farm-y',y+'%');highlight(dock,valid);e.preventDefault();};
    const finish=e=>{if(e.pointerId!==pointerId)return;document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',finish);document.removeEventListener('pointercancel',finish);try{handle.releasePointerCapture(pointerId);}catch(_){}plot.classList.remove('dragging','invalid');map.classList.remove('farm-dragging');map.querySelectorAll('.nr-farm-dock').forEach(el=>el.classList.remove('active','source'));if(!moved)return;if(!valid){plot.style.setProperty('--nr-farm-x',start.x+'%');plot.style.setProperty('--nr-farm-y',start.y+'%');if(typeof showToast==='function')showToast('Ô này đã có nông trại — chọn ô vuông sáng khác quanh lâu đài');return;}farm.x=x;farm.y=y;appState.nightRaidLayout=NightRaidRules.normalizeLayout(layout);builderSuppressClick=true;setTimeout(()=>{builderSuppressClick=false;},160);save();syncHome();announce('Nông trại '+zone+' đã bắt dính vào ô cạnh lâu đài');rememberBuilderWorld();renderBuilder();};
  ...
  async function showReports(){cleanup();setNav(true);view='reports';const r=root();if(!r)return;
    ...
    const attackSection=attacks?`<section class="nr-log-section"><div class="nr-live-head"><h3>⚔️ Con đi cướp</h3><span>${attacks.length} trận</span></div><div class="nr-report-list">${attacks.map(attackCardHTML).join('')||'<div class="nr-empty-state">'+svg('moon')+'<h3>Con chưa đi cướp nhà nào</h3><p>Bấm ĐI CƯỚP ở màn hình nhà rồi chọn một nhà để bắt đầu.</p></div>'}</div></section>`:'';
...
function nrBuyFarmPlot(style){NightRaid.buyFarmPlot(style,false);}
function nrBuildCell(lane,col){NightRaid.buildCell(lane,col);}
function nrGridCell(gx,gy,zone){NightRaid.gridCell(gx,gy,zone);}
...
function nrToggleBuilderMenu(){NightRaid.toggleBuilderMenu();}
...
function nrDropBuildItem(event,gx,gy,zone){NightRaid.dropBuildItem(event,gx,gy,zone);}
function nrBeginPlacedDrag(event,gx,gy,layer,zone){NightRaid.beginPlacedDrag(event,gx,gy,layer,zone);}
function nrBeginFarmPlotDrag(event,zone){NightRaid.beginFarmPlotDrag(event,zone);}
