...
var NightRaid = (() => {
  ...
  function winRewardHTML(state,reward){return reward>0?`${svg('coin')}<span>+${reward} xu đã cướp</span>`:`${svg('coin')}<span>${zeroLootText(state)}</span>`;}
  ...
  function buildCell(gx,gy,confirmed,zone){if(!confirmed&&builderSuppressClick){builderSuppressClick=false;return;}if(!builderEditing&&!confirmed)return;const plan=buildPurchasePlan(selectedBuild,gx,gy,zone);if(!plan)return;if(plan.error){if(typeof showToast==='function')showToast(plan.error);return;}if(!confirmed){showBuildPurchase(plan);return;}if(plan.def.kind==='crop')return plantSeed(plan);const {def,layout,at,cell,cost,refund}=plan,z=zoneOf(layout,plan.zone),G=z.grid,lane=Math.round(plan.gy*4/(G-1)),col=1+Math.round(plan.gx*7/(G-1));
