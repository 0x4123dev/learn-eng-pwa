...
var NightRaid = (() => {
  ...
  function yardArmyHtml(){const count=Math.max(0,Math.min(NightRaidRules.MAX_SOLDIERS,Math.trunc(+appState.nightRaidLayout?.soldiers||0)));if(!count)return'';return `<div class="nr-yard-army" data-nr-yard-army data-mode="march" aria-label="Đội hình ${count} lính đang duyệt binh trên bãi cỏ"><div class="nr-army-trail" data-nr-army-trail aria-hidden="true"></div>${armySlots(count).map((s,i)=>`<i class="nr-home-soldier" data-unit="${i}" data-row="${s.row}" style="--nr-slot-x:${s.x}%;--nr-slot-y:${s.y}%;--nr-unit-position:0% ${s.row*20}%"><b aria-hidden="true"></b></i>`).join('')}</div>`;}
