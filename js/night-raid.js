...
var NightRaid = (() => {
  ...
  async function syncHome(){const body={layout:appState.nightRaidLayout,teammates:appState.battleTeammates,castleSkin:appState.petBattleCastleSkin||'stone-keep'};if(finite(appState.dogLevel))body.dogLevel=appState.dogLevel;if(finite(appState.coins))body.coins=appState.coins;if(finite(appState.vaultCoins))body.vaultCoins=appState.vaultCoins;const res=await api('home',{method:'PUT',body});if(res.ok&&res.data?.layout)appState.nightRaidLayout=NightRaidRules.normalizeLayout(res.data.layout);return res;}
