...
var NightRaid = (() => {
  ...
  function buyFarmPlot(confirmed){const P=NightRaidRules.farmRules.FARM_PLOT,layout=NightRaidRules.normalizeLayout(appState.nightRaidLayout),have=layout.farms.length,balance=Math.max(0,Math.floor(+appState.coins||0));
    ...
    const pos=NightRaidRules.FARM_PLOT_POSITIONS[have]||{x:-42,y:-55};
    appState.coins=balance-P.price;layout.farms.push({cells:[],x:pos.x,y:pos.y});appState.nightRaidLayout=NightRaidRules.normalizeLayout(layout);builderZone=0;builderFocusFarm=layout.farms.length;builderScroll=null;builderScrollByView.builder=null;builderShopTab='seeds';builderEditing=true;builderShopOpen=false;save();syncHome();if(typeof showToast==='function')showToast('Đã mua '+P.name.vi+' · kéo bảng NÔNG TRẠI để đặt quanh lâu đài');announce('Đã mua nông trại riêng. Kéo bảng tên để chuyển vị trí.');renderBuilder();}
  ...
  function mountUnifiedFarms(map,layout){if(!map)return;const main=map.querySelector(':scope > .nr-free-grid');if(main){main.dataset.zone='0';main.querySelectorAll('.nr-build-grid-cell').forEach(cell=>cell.dataset.zone='0');}
    map.querySelectorAll(':scope > .nr-farm-plot').forEach(plot=>plot.remove());
    (layout.farms||[]).forEach((farm,index)=>{const zone=index+1;map.insertAdjacentHTML('beforeend',`<section class="nr-farm-plot" data-farm-zone="${zone}" style="--nr-farm-x:${farm.x}%;--nr-farm-y:${farm.y}%" aria-label="Nông trại ${zone}, cùng bản đồ với lâu đài"><img src="img/farm/farm-plot.webp" alt="Mảnh đất Nông trại ${zone}"><button type="button" class="nr-farm-plot-handle" onpointerdown="nrBeginFarmPlotDrag(event,${zone})" aria-label="Kéo Nông trại ${zone} đến vùng đất trống">🌱 <span>NÔNG TRẠI ${zone} · KÉO</span></button><div class="nr-free-grid size-6" data-zone="${zone}" role="grid" aria-label="Lưới Nông trại ${zone}, 6 nhân 6">${farmGridHtml(layout,zone)}</div></section>`);});
  }
  ...
  function showBuildPurchase(plan){
    pendingBuildPurchase={id:plan.id,gx:plan.gx,gy:plan.gy,zone:plan.zone};document.querySelector('.nr-purchase-backdrop')?.remove();const builder=document.querySelector('.nr-builder');if(!builder)return;
  ...
  function confirmBuildPurchase(){const pending=pendingBuildPurchase;if(!pending)return;pendingBuildPurchase=null;document.querySelector('.nr-purchase-backdrop')?.remove();if(pending.id==='farm-plot'){buyFarmPlot(true);return;}selectedBuild=pending.id;buildCell(pending.gx,pending.gy,true,pending.zone);}
  ...
  function beginFarmPlotDrag(event,zone){if(!builderEditing||(event.pointerType==='mouse'&&event.button!==0))return;event.preventDefault();event.stopPropagation();zone=Math.max(1,Math.trunc(+zone||0));const handle=event.currentTarget,plot=handle.closest('.nr-farm-plot'),map=plot?.closest('.nr-builder-map');if(!plot||!map)return;
    const layout=NightRaidRules.normalizeLayout(appState.nightRaidLayout),farm=layout.farms[zone-1];if(!farm)return;const bounds=NightRaidRules.FARM_PLOT_BOUNDS,pointerId=event.pointerId,startX=event.clientX,startY=event.clientY,start={x:farm.x,y:farm.y};let x=start.x,y=start.y,moved=false,valid=true;
    const overlaps=(ax,ay,aw,ah,bx,by,bw,bh)=>ax<bx+bw&&ax+aw>bx&&ay<by+bh&&ay+ah>by;
    const canPlace=(nx,ny)=>!overlaps(nx,ny,38,51,0,0,100,100)&&layout.farms.every((other,index)=>index===zone-1||!overlaps(nx,ny,38,51,other.x,other.y,38,51));
    try{handle.setPointerCapture(pointerId);}catch(_){}plot.classList.add('dragging');
    const move=e=>{if(e.pointerId!==pointerId)return;const rect=map.getBoundingClientRect();if(Math.hypot(e.clientX-startX,e.clientY-startY)>4)moved=true;if(!moved)return;x=Math.max(bounds.minX,Math.min(bounds.maxX,Math.round(start.x+(e.clientX-startX)/Math.max(1,rect.width)*100)));y=Math.max(bounds.minY,Math.min(bounds.maxY,Math.round(start.y+(e.clientY-startY)/Math.max(1,rect.height)*100)));valid=canPlace(x,y);plot.style.setProperty('--nr-farm-x',x+'%');plot.style.setProperty('--nr-farm-y',y+'%');plot.classList.toggle('invalid',!valid);e.preventDefault();};
    const finish=e=>{if(e.pointerId!==pointerId)return;document.removeEventListener('pointermove',move);document.removeEventListener('pointerup',finish);document.removeEventListener('pointercancel',finish);try{handle.releasePointerCapture(pointerId);}catch(_){}plot.classList.remove('dragging','invalid');if(!moved)return;if(!valid){plot.style.setProperty('--nr-farm-x',start.x+'%');plot.style.setProperty('--nr-farm-y',start.y+'%');if(typeof showToast==='function')showToast('Nông trại cần một vùng cỏ trống, không chồng lên lâu đài hoặc nông trại khác');return;}farm.x=x;farm.y=y;appState.nightRaidLayout=NightRaidRules.normalizeLayout(layout);builderSuppressClick=true;setTimeout(()=>{builderSuppressClick=false;},160);save();syncHome();announce('Đã đặt Nông trại '+zone+' quanh lâu đài');rememberBuilderWorld();renderBuilder();};
  ...
  async function showLiveTargets(){cleanup();setNav(true);view='live';const r=root();if(!r)return;
    ...
    r.innerHTML=shell(`<main class="nr-live-targets">${overview(tickets,'Chạm một lâu đài để trinh sát')}${ownStatusHtml(me)}<div class="nr-live-head"><h3>👥 Bạn bè</h3><span>${friends.length} nhà</span></div>${friendList}</main>`);
