...
var NightRaid = (() => {
  ...
  function shopTabsHtml(zone){const tabs=SHOP_TABS.filter(([id])=>zone.zone===0||id!=='defense');return `<div class="nr-shop-tabs" role="tablist" aria-label="Loại vật phẩm">${tabs.map(([id,label,icon])=>`<button type="button" role="tab" class="${builderShopTab===id?'active':''}" aria-label="${label}" title="${label}" aria-selected="${builderShopTab===id}" onclick="nrSelectShopTab('${id}')">${svg(icon)}</button>`).join('')}</div>`;}
  function trayHtml(layout,zone){if(zone.zone>0&&builderShopTab==='defense')builderShopTab='farm';if(builderShopTab==='seeds')return NightRaidRules.farmRules.CROPS.map(seedCardHtml).join('');if(builderShopTab==='expand')return expandCardHtml(layout);return shopItems(builderShopTab).map(d=>shopCardHtml(d,layout)).join('');}
  ...
  function showBuildPurchase(plan){
    pendingBuildPurchase={id:plan.id,gx:plan.gx,gy:plan.gy};document.querySelector('.nr-purchase-backdrop')?.remove();const builder=document.querySelector('.nr-builder');if(!builder)return;
